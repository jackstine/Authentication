...
describe("Authentication", function () {
  ...
  describe("#createAuthentication", function () {
    it("should create the authentication object", function (done) {
      createAuthentication({ plugin: PluginMock, googleClientId: "this is the client id" })
        .then((resp) => {
          expect(auth.users).to.not.be.undefined;
          expect(auth.token).to.not.be.undefined;
          expect(auth.token.googleClientId).to.not.be.undefined;
          expect(auth.token.googleSignin).to.not.be.undefined;
          done();
        })
        .catch(console.error);
    });
